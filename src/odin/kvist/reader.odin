...
is_delimiter :: proc(ch: byte) -> bool {
    return is_whitespace(ch) || ch == '(' || ch == ')' || ch == '[' || ch == ']' || ch == '{' || ch == '}' || ch == ',' || ch == ';' || ch == '\'' || ch == '`' || ch == '~'
}

is_symbol_boundary :: proc(ch: byte) -> bool {
    return is_whitespace(ch) || ch == '(' || ch == ')' || ch == '{' || ch == '}' || ch == ',' || ch == ';' || ch == '\'' || ch == '`' || ch == '~'
}
...
tokenize_with_origin :: proc(source: string, source_kind: Source_Kind) -> (tokens: [dynamic]Token, err: Compile_Error, ok: bool) {
    ...
    for i < len(source) {
        ...
        case '}':
            ...
            continue
        case '\'':
            append(&tokens, make_token(.Quote, "'", start, start+1, source_kind))
            i += 1
            continue
        case '`':
            append(&tokens, make_token(.Quasiquote, "`", start, start+1, source_kind))
            i += 1
            continue
        case '~':
            if i+1 < len(source) && source[i+1] == '@' {
                append(&tokens, make_token(.Splice, "~@", start, start+2, source_kind))
                i += 2
                continue
            }
            append(&tokens, make_token(.Unquote, "~", start, start+1, source_kind))
            i += 1
            continue
        case '"':
            i += 1
            ...
            if i+1 < len(source) && source[i+1] == '"' {
                ...
                for i < len(source) {
                    ...
                    if escaped {
                        ...
                        i += 1
                        append(&tokens, make_token(.Regex, source[start:i], start, i, source_kind))
                        break
                    }
                    i += 1
                }
                if len(tokens) == 0 || tokens[len(tokens)-1].span.start != start {
                    return tokens, Compile_Error{message = "unterminated regex literal", span = make_span(start, len(source), source_kind)}, false
                }
                continue
            }
            if i+1 < len(source) && source[i+1] == '{' {
...
reader_syntax_form :: proc(head: string, token: Token, item: CST_Form) -> CST_Form {
    items: [dynamic]CST_Form
    append(&items, CST_Form{kind = .Symbol, text = head, span = token.span})
    append(&items, item)
    return CST_Form{
        kind = .List,
        items = items,
        span = make_span(token.span.start, item.span.end, token.span.source),
    }
}
...
parse_primary_form :: proc(tokens: []Token, index: ^int) -> (form: CST_Form, err: Compile_Error, ok: bool) {
    ...
    case .Regex:
        index^ += 1
        return CST_Form{kind = .Regex, text = token.text, span = token.span}, {}, true
    case .Number:
        index^ += 1
    ...
    case .Quote:
        ...
        return reader_syntax_form("quote", token, item), {}, true
    case .Quasiquote:
        index^ += 1
        item, err_item, ok_item := parse_form(tokens, index)
        if !ok_item {
        ...
        return reader_syntax_form("quasiquote", token, item), {}, true
    case .Unquote:
        index^ += 1
        item, err_item, ok_item := parse_form(tokens, index)
        if !ok_item {
        ...
        return reader_syntax_form("unquote", token, item), {}, true
    case .Splice:
        index^ += 1
        item, err_item, ok_item := parse_form(tokens, index)
        if !ok_item {
        ...
        return reader_syntax_form("splice", token, item), {}, true
    case .Line_Comment:
        index^ += 1
...
unquote_regex_literal :: proc(text: string) -> string {
